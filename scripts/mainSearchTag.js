...
export function displayFilteredRecipes(searchQuery = "", selectedIngredients = [], selectedAppliances = [], selectedUstensiles = []) {
    ...
    const filteredRecipes = recipes.filter(recipe => {
        ...
        const hasMatchingAppliance = selectedAppliances.length === 0 || 
            selectedAppliances.includes(recipe.appliance.toLowerCase());

        // Vérification des ustensiles sélectionnés
        const hasMatchingUstensils = selectedUstensiles.length === 0 || 
        ...
        const matchesSearch = trimmedQuery.length < 3 || (
